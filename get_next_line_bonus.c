...
{
	char		*str;
	static char	*reader_static[1024];
	int			reads;
	char		*output;
	...
	if (!str)
		return (NULL);
	while (size_line(reader_static[fd]) == -1)
	{
		reads = read(fd, str, BUFFER_SIZE);
		if (reads <= 0)
			break ;
		str[reads] = '\0';
		reader_static[fd] = ft_strjoin(reader_static[fd], str);
	}
	free(str);
	output = line(reader_static[fd]);
	reader_static[fd] = remove_d(reader_static[fd]);
	return (output);
}
